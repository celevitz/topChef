...
# 1. Check that there aren't NAs where there shouldn't be
  ## A. Chef Details
    ...
    # and placement in season 20
      ...
      test_that("Everyone has a placement except in Season 20",
                {expect_equal(all(!(is.na(chefdetails$placement[chefdetails$szn
                                              != "World All Stars"]))),TRUE)})
  ## B. Challenge descriptions
      ...
      for (varname in c("szn","sznnumber","series","episode","challenge_type"
                        ,"outcome_type")) {
  ...
  ## C. Challenge wins
      ...
      for (varname in c("szn","sznnumber","series","episode","in.competition"
                        ,"chef","challenge_type")) {
  ...
  ## D. Episode info
        ...
        for (varname in c("szn","sznnumber","series","episode"
                          ,"#.of.competitors")) {
  ...
  ## F. Rewards
        ## NAs are not okay
        for (varname in c("szn","sznnumber","series","episode","challenge_type"
                          ,"outcome_type","reward_type","reward","chef")) {
...
## 2. Test Index Calculation
  ## A. Does it output a data frame?
        test_that("index calculation returns a data frame", {
          indexoutput <- weightedindex(seriesname="US",seasonnumber=20
